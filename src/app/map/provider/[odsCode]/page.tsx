...
import { useProvider } from '@/map/hooks/useProvider';
...
export default function ProviderPage({ params }: { params: Promise<{ odsCode: string }> }) {
  const { odsCode } = use(params);
  const { provider, loading, error } = useProvider(odsCode);

  if (loading) return <div className="min-h-screen bg-cp-bg flex items-center justify-center"><span className="text-cp-text-muted text-sm font-medium">Loading...</span></div>;

  if (error || !provider) return (
    <div className="min-h-screen bg-cp-bg">
      <div className="max-w-2xl mx-auto px-6 py-10">
        <Link href="/map" className="text-cp-dark text-sm font-semibold hover:opacity-70">&larr; Back to search</Link>
        <div className="mt-6 px-4 py-3 bg-red-50 border border-red-100 rounded-2xl text-red-600 text-sm">{error || 'Hospital not found'}</div>
      </div>
    </div>
  );
  ...
  const periodKey = (p: string) => { const [month, year] = p.split(' '); return (parseInt(year) || 0) * 100 + (monthOrder[month] || 0); };
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="mt-6 bg-cp-surface border-[1.5px] border-cp-border rounded-[20px] p-6">
          ...
          <div className="flex flex-wrap gap-3 mt-4">
            <a href={directionsUrl(provider.latitude, provider.longitude)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-cp-dark text-white text-[13px] font-bold hover:opacity-90 transition-opacity">
        ...
        {latestData && (
          <div className="mt-6 space-y-4">
            ...
            <div className="grid gap-4">
              {avgFds !== undefined && <MetricCard title="Time to diagnosis" subtitle="How quickly you'll find out your results after referral" days={perfToAvgDays(avgFds, 28)} target={28} performance={avgFds} />}
              {avg31d !== undefined && <MetricCard title="Time to start treatment" subtitle="How quickly treatment begins once decided" days={perfToAvgDays(avg31d, 31)} target={31} performance={avg31d} />}
              {avg62d !== undefined && <MetricCard title="Referral to treatment" subtitle="Total time from GP referral to first treatment" days={perfToAvgDays(avg62d, 62)} target={62} performance={avg62d} />}
            </div>
          </div>
        )}
          ...
          <div className="mt-8">
            ...
            <div className="bg-cp-surface border-[1.5px] border-cp-border rounded-[20px] p-5">
              <div className="flex flex-wrap gap-2">
                {fdsEntries.map(e => <span key={e.cancer_type} className="px-3.5 py-1.5 rounded-full bg-cp-bg text-[13px] text-cp-dark font-medium border-[1.5px] border-cp-border">{e.display_name}</span>)}
              </div>
            </div>
          </div>
        )}
        ...
        <div className="mt-6 text-[12px] text-cp-text-muted opacity-60 leading-relaxed">Data from NHS England Cancer Waiting Times ({latestPeriod}). Wait times are indicative averages based on monthly published statistics. Actual wait times may vary. Always discuss your care options with your GP.</div>
      </div>
    </div>
  );
}
